(function ($, skuid, undefined)
{
	...
	$b.registerBuilder(new $b.Builder(
	{
		...
		handleStateEvent: function (c, event, f)
		{
			...
			changeEvent && $bc.handleModelIdChange(c, event);
			(removeEvent || changeEvent) && c.children("actions").first().children('action[type="savecancel"]').each(function ()
			{
				$(this).children("models").first().children("model").each(function ()
				{
					var a = $(this).text();
					if (removeEvent && a === event.modelId)
					{
						$(this).remove();
						update = true;
					}
					...
					{
						changeEvent && (a === event.oldId) && $(this).text(event.newId);
						update = true;
					}
				});
			});
